...
const ENV_KEY = 'GGUF_HOST';

export class GGUFProvider extends BaseAIProvider {
  readonly name = 'gguf';
  readonly displayName = 'Local GGUF';
  ...
  async complete(req: AICompletionRequest): Promise<AICompletionResponse> {
    try {
      ...
      const d = r.data;
      return { id: d.id, content: d.choices[0].message.content, model: d.model, provider: 'gguf', tokens: { input: d.usage?.prompt_tokens || 0, output: d.usage?.completion_tokens || 0, total: d.usage?.total_tokens || 0 }, finishReason: d.choices[0].finish_reason || 'stop' };
    } catch (error) {
      if (axios.isAxiosError(error)) throw new CodeStrikeError(`GGUF error: ${error.response?.data?.error?.message || error.message}`, 'AI_PROVIDER_ERROR', error.response?.status || 500);
      throw error;
    }
  }
  ...
  async *stream(req: AICompletionRequest): AsyncGenerator<AIStreamChunk> {
    try {
      ...
      if (axios.isAxiosError(error)) throw new CodeStrikeError(`GGUF stream error: ${error.message}`, 'AI_PROVIDER_ERROR', 500);
      throw error;
    }
  }
  ...
  async validateConfig(): Promise<boolean> {
    try { await axios.get(`${this.baseUrl}/models`, { timeout: 5000 }); return true; } catch { return false; }
  }
}
