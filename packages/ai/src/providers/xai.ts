...
export class XAIProvider extends BaseAIProvider {
  ...
  async complete(req: AICompletionRequest): Promise<AICompletionResponse> {
    try {
      ...
      const d = r.data;
      return { id: d.id, content: d.choices[0].message.content, model: d.model, provider: 'xai', tokens: { input: d.usage?.prompt_tokens || 0, output: d.usage?.completion_tokens || 0, total: d.usage?.total_tokens || 0 }, finishReason: d.choices[0].finish_reason || 'stop' };
    } catch (error) {
      if (axios.isAxiosError(error)) throw new CodeStrikeError(`xAI error: ${error.response?.data?.error?.message || error.message}`, 'AI_PROVIDER_ERROR', error.response?.status || 500);
      throw error;
    }
  }
  ...
  async *stream(req: AICompletionRequest): AsyncGenerator<AIStreamChunk> {
    try {
      ...
      if (axios.isAxiosError(error)) throw new CodeStrikeError(`xAI stream error: ${error.message}`, 'AI_PROVIDER_ERROR', 500);
      throw error;
    }
  }
